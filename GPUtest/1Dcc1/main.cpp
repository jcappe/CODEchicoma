...
int main() 
{
    ...
    CUFFT_CALL(cufftPlan1d(&plan, data.size(), CUFFT_C2C, 1));
    ...
    CUDA_RT_CALL(cudaMalloc(reinterpret_cast<void **>(&d_data), sizeof(std::complex<float>) * data.size()));
    CUDA_RT_CALL(cudaMemcpyAsync(d_data, data.data(), sizeof(std::complex<float>) * data.size(), cudaMemcpyHostToDevice, stream));
    ...
    //CUDA_RT_CALL(cudaMemcpyAsync(data.data(), d_data, sizeof(std::complex<float>) * data.size(), cudaMemcpyDeviceToHost, stream));      // put copy here if want to see transform data on GPU
    ...
    CUDA_RT_CALL(cudaMemcpyAsync(data.data(), d_data, sizeof(std::complex<float>) * data.size(), cudaMemcpyDeviceToHost, stream));     // put copy here if want to see inverse transform data (should be back to original intput multiplied by number of elements)
