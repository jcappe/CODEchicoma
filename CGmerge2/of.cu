...
#include "/users/jcap/CUDAex/CUDALibrarySamples/cuFFTMp/samples/common/error_checks.hpp"
#include "/users/jcap/CUDAex/CUDALibrarySamples/cuFFTMp/samples/iterators/box_iterator.hpp"
...
__global__ void hartree_energy(COMPLEX *d_rho_R, COMPLEX *d_hartree_pot_desc, REAL *d_hartree_pot, REAL *d_elemental_hartree_energy, int data_size, REAL dx, REAL dy, REAL dz)
{
    const int tid_x = threadIdx.x + blockIdx.x * blockDim.x; 
...
void calc_vw(size_t my_data_size, size_t nx, size_t ny, size_t nz, cudaLibXtDesc *psi_G_desc, REAL *d_vw_pot, REAL *d_el_kinetic_energy,
                cudaLibXtDesc *desc_kx, cudaLibXtDesc *desc_ky, cudaLibXtDesc *desc_kz, int rank, int size, MPI_Comm comm, unsigned int plan, REAL Lx, REAL Ly, REAL Lz, REAL cutoff, REAL tf_lambda)
{
                ...
                cudaLibXtDesc *desc_kx, cudaLibXtDesc *desc_ky, cudaLibXtDesc *desc_kz, int rank, int size, MPI_Comm comm, unsigned int plan, REAL dx, REAL dy, REAL dz)
{
    ...
    hartree_energy<<<num_blocks, num_threads>>>((COMPLEX *)rho_R_desc->descriptor->data[0], (COMPLEX *)hartree_desc->descriptor->data[0], 
...
__global__ void calc_xc_vwn(COMPLEX *d_rho_R, REAL *d_vxc, REAL *d_exc, REAL *d_elemental_xc_energy, size_t my_data_size,  
...
void calc_xc(size_t my_data_size, COMPLEX *d_rho_R, REAL *d_xc_pot, REAL *d_el_xc_energy, int rank, REAL dx, REAL dy, REAL dz, REAL c_light)
{
    REAL y0 = -0.10498;
    REAL b = 3.72744;
    REAL c = 12.9352;
